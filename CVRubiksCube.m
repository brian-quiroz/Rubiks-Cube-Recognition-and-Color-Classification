...
function test(test2run, t)
    if(test2run == "all")
        testMode("scale", t);
        testMode("translation", t);
        testMode("rotation", t);
        testMode("background", t);
        testMode("lighting", t);
        test("color-all", t);
    elseif (test2run == "scale")
        testMode("scale", t);
    elseif (test2run == "translation")
        testMode("translation", t);
    elseif (test2run == "rotation")
        testMode("rotation", t);
    elseif (test2run == "background")
        testMode("background", t);
    elseif (test2run == "lighting")
        testMode("lighting", t);
    elseif (test2run == "color-all")
        run(1, 1, t);
        run(2, 1, t);
        run(3, 1, t);
    elseif (test2run == "color1")
        run(1, 1, t);
    elseif (test2run == "color2")
        run(2, 1, t);
    elseif (test2run == "color3")
        run(3, 1, t);
    else
        disp("Error! Please input correct test name!");
    end
end

function testMode(mode, t)
    n = 15;
    f = 182.5;
    m = 150;
    flag = 0;
    ...
    disp(mode + " test starting...");
    
    for l = 1:6
       fileName = "Images/ImgTest-" + mode + "/Img" + l + ".jpg";
       I = imread(fileName);
       [preRotI, rotI, lines] = detectEdges(I, t, n, f, m);
       plotEdges(rotI, lines, l, flag);
    end
    
    title = mode + " test with t = " + t;
    sgtitle(title);
    saveas(gcf, "Experiments/" + title + ".jpg");
    close;
    disp(mode + " test finished successfully!");
    disp(" ");
end

function run(setN, isTest, t)
    n = 15;
    f = 182.5;
    m = 150;
    flag = 1;
    ...
    for l = 1:6
        if (isTest == 1)
            fileName = "Images/ImgTest-color" + setN + "/Img" + l + ".jpg";
        else
            fileName = "Images/ImgRun-set" + setN + "/Img" + l + ".jpg";
        end
        I = imread(fileName);
        [preRotI, rotI, lines] = detectEdges(I, t, n, f, m);
    ...
    if (mse < 10000)
        ...
        for l = 1:6
            fileName = "CroppedImage/Img" + l + ".jpg";
            I = imread(fileName);
        ...
        if (isTest == 1)
            fileID = fopen("Experiments/stickers-color"+ setN + ".txt",'w');
        else
            fileID = fopen("stickers-set"+ setN + ".txt",'w');
        end
...
function findROIs(I, preRotI, c1, c2, c3, c4, n)
    ...
    saveas(gcf, "CroppedImage/Img" + int2str(n) + ".jpg");
    close;
end
